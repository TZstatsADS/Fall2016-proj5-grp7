...
ride.counts.filter.threshold <- ride.counts.all %>% dplyr::filter(total.count >= 5000)
...
paint.arrows <- function(map, arrow.data){
  ...
  for(i in 1:nrow(arrow.data)){
    ...
    # Paint line
    map <- addPolylines(map, lng = lng.x, lat = lat.x, weight = line.weight, color = arrow.data[i,5])
