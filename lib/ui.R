...
sidebarMenu(
  ...
  menuItem("Connections", icon = icon("cog"),tabName="Connections")
)
)
...
body <- dashboardBody(
  ...
  tabItems(
    tabItem(tabName = "Map",
            leafletOutput("map3", width = "100%", height = 900),
            # leafletOutput("map3"),
            # plotOutput("map2"),
            div(class="outer",tags$head(includeCSS("styles.css")),
                absolutePanel(id = "controls",class = "panel panel-default", fixed = TRUE,
                              draggable = FALSE, top = "auto", left = "auto", right = 15, bottom = 0,
    ...
    # tabItem(tabName = "Connections", column(4,includeHTML("include.md"),tags$a(href="http://rpubs.com/zuozuo/233483", "Click Here!")
    # ))
    )
)
